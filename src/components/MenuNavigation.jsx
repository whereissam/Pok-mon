    ...
    <>
      <BrowserView>
        <div className="shadow-md w-full fixed top-0 left-0 z-30">
          <ul className="flex justify-between w-full items-center bg-black px-7 py-4 md:px-10 transition-all duration-500 ease-in">
            <div className="relative flex flex-wrap w-10/12 justify-around md:justify-evenly">
              {Links.map(link => (
                <a
                  key={link.name}
                  href={link.link}
            ...
            {/* Sign In button */}
            <Button className="hidden md:block w-2/12 text-[#F7BC1A]">
              SIGN IN
            </Button>
          </ul>
        </div>
